...
+ (NSDictionary *)credentials {
	...
	if (status == noErr && foundValue) {
		...
		if (status != errSecItemNotFound) {
			DBLogWarning(@"DropboxSDK: error reading stored credentails (%d)", (int)status);
		}
		return nil;
	}
}
...
+ (void)setCredentials:(NSDictionary *)credentials {
	...
	if (status != noErr) {
		DBLogWarning(@"DropboxSDK: error saving credentails (%d)", (int)status);
	}
}
...
+ (void)deleteCredentials {
	...
	if (status != noErr) {
		DBLogWarning(@"DropboxSDK: error deleting credentials (%d)", (int)status);
	}
}
