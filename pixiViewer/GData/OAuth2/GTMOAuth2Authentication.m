  ...
  if (serializer) {
    ...
    if (jsonParseClass) {
      ...
      NSString *jsonStr = [[[NSString alloc] initWithData:data
                                                 encoding:NSUTF8StringEncoding] autorelease];
      if (jsonStr) {
		  if ([jsonStr respondsToSelector:@selector(JSONValue)]) {
			  obj = [jsonStr performSelector:@selector(JSONValue)];
		  } else if ([parser respondsToSelector:@selector(objectWithString:error:)]) {
