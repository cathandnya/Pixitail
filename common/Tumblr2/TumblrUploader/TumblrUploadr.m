...
static NSComparisonResult paramCompare(id obj1, id obj2, void *ref) {
...
- (id)initWithNSDataForPhotos:(NSArray *)aPhotoDataArray andBlogName:(NSString *)aBlogName andDelegate:(id)aDelegate {
    [self initWithNSDataForPhotos:aPhotoDataArray andBlogName:aBlogName andDelegate:aDelegate andCaption:nil andTags:nil andLink:nil otherParam:nil];
    return self;
}

- (id)initWithNSDataForPhotos:(NSArray *)aPhotoDataArray andBlogName:(NSString *)aBlogName andDelegate:(id)aDelegate andCaption:(NSString *)aCaption andTags:(NSString *)tags andLink:(NSString *)link otherParam:(NSDictionary *)paramDic {
    self = [super init];
    if (self) {
        ...
        for (NSData *photoData in photoDataArray) {
            NSString *thisKey = [NSString stringWithFormat:@"data%%5B%d%%5D",(int)[photoDataArray indexOfObject:photoData]];
        ...
        if (tags !=nil) {
            [params addObject:[NSDictionary dictionaryWithObjectsAndKeys:@"tags", @"key", [tags encodeForURL], @"value", nil]];
        }
        if (link !=nil) {
            [params addObject:[NSDictionary dictionaryWithObjectsAndKeys:@"link", @"key", [link encodeForURL], @"value", nil]];
        }
		for (NSString *key in [paramDic allKeys]) {
			NSString *val = [paramDic objectForKey:key];
			[params addObject:[NSDictionary dictionaryWithObjectsAndKeys:key, @"key", [val encodeForURL], @"value", nil]];
		}
...
- (id)initWithData:(NSData *)data forType:(NSString *)type andBlogName:(NSString *)aBlogName andDelegate:(id)aDelegate andCaption:(NSString *)aCaption  {
    self = [super init];
    if (self) {
        self.delegate = aDelegate;
        self.blogName = aBlogName;
        self.photoDataArray = [NSArray arrayWithObject:data];
        self.caption = aCaption;
        self.url = [NSURL URLWithString:[NSString stringWithFormat:@"http://api.tumblr.com/v2/blog/%@/post",[self blogName]]];
        NSMutableData *someResponseData = [NSMutableData data];
        [self setResponseData:someResponseData];
        ...
        NSMutableArray *someParams = [NSMutableArray arrayWithCapacity:[photoDataArray count]+1];
        [self setParams:someParams];
		[params addObject:[NSDictionary dictionaryWithObjectsAndKeys:@"data", @"key", [data stringWithoutURLEncoding], @"value", nil]];
        [params addObject:[NSDictionary dictionaryWithObjectsAndKeys:@"type", @"key", type, @"value", nil]];
        if (self.caption !=nil) {
            [params addObject:[NSDictionary dictionaryWithObjectsAndKeys:@"caption", @"key", [self.caption encodeForURL], @"value", nil]];
        }
        //    [request setHTTPBody:XXX];
...
- (NSString *)oauthRequestParameterString:(NSArray *)oauthParameters
{
    ...
    */
	...
	[parameters sortUsingFunction:paramCompare context:nil];
	/*
